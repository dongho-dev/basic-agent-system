> 기회/신기능 발견 → 이슈 생성 | context_tier: 1m
...
1. 이슈 목록(body 포함)을 이후 모든 발견과 대조한다. 이미 이슈가 있는 아이디어는 건너뛴다.
...
3. README, 문서 파일에서 프로젝트 목적·도메인 파악
...
프로젝트의 주요 기능 코드를 탐색하여 **빠져 있는 기능, 불완전한 기능, 사용자 경험을 향상시킬 기회**를 찾는다.

탐색 관점 (프로젝트 유형에 맞게 적용):
- 구현된 기능 중 빠진 부분 (예: 생성은 있지만 수정/삭제 없음, 입력은 되지만 출력/내보내기 없음)
- 사용자에게 노출되지 않는 유용한 데이터/기능
- 에러/빈 상태에 대한 피드백 부재
- 유사 프로젝트가 보통 제공하는 기능 대비 빠진 것
- 설정 가능하면 좋을 하드코딩된 값

### Agent 2: DX + 자동화 기회 (dx)

프로젝트의 빌드 설정, 스크립트, CI, 타입 정의, 개발 워크플로우를 탐색하여 **개발 경험·자동화를 개선할 기회**를 찾는다.

탐색 관점 (프로젝트 유형에 맞게 적용):
- 반복적인 수동 작업을 자동화할 수 있는 것
- 타입 안전성을 높일 수 있는 곳
- 빌드/배포/릴리스 파이프라인 개선 여지
- 로깅/모니터링/디버깅 도구 부재
- 문서화 부재 (API 문서, 사용 가이드 등)

### Agent 3: 아키텍처 진화 기회 (architecture)

프로젝트의 전체 구조, 모듈 간 의존성, 데이터 흐름을 탐색하여 **아키텍처 개선·확장성 확보 기회**를 찾는다.

탐색 관점 (프로젝트 유형에 맞게 적용):
- 모듈 간 과도한 결합 (하나 고치면 여러 곳이 깨지는 구조)
- 확장 가능하지 않은 패턴 (하드코딩된 분기, 매직 넘버 등)
- 캐싱/최적화로 성능을 크게 개선할 수 있는 핫 경로
- 도입하면 좋을 패턴 (추상화, 플러그인 구조, 이벤트 시스템 등)
- 데이터 모델/인터페이스 확장 여지
...
| 이슈 | 제목 | 영역 | 임팩트 | 라벨 |
|------|------|------|--------|------|
| #501 | ... | feature | 높음 | feature |
| #502 | ... | dx | 중간 | enhancement |
