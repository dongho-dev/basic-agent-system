...
- **불채택 근거:** Claude Code 자체가 MCP 클라이언트. 우리 커맨드 파일은 Claude Code 프롬프트이므로 MCP 도구를 직접 사용 가능. 별도 MCP 허브 레이어는 **불필요한 간접층**. Ouroboros는 Claude Code 외에 Codex CLI도 지원해야 하므로 추상화가 필요했지만, 우리는 Claude Code 기반 + Codex 플러그인으로 GPT 모델을 리뷰 파이프라인에 통합 (별도 오케스트레이션 추상화 불필요)
...
- **불채택 근거:** 우리는 **Claude Code 기반**. 별도 에이전트 런타임(Aider/OpenCode) 지원은 불필요. Codex는 Claude Code의 플러그인(`codex:codex-rescue`)으로 통합하여 리뷰 파이프라인에서 GPT 모델을 활용. 플러그인 아키텍처 수준의 추상화 없이 cross-model 앙상블 달성
...
- **불채택 근거:** 우리는 Claude Code 기반. pipeline-cli.mjs + Claude Code slash command 아키텍처가 다른 플랫폼과 호환되지 않음. Codex는 플랫폼이 아닌 모델로서 Claude Code 플러그인을 통해 통합. 멀티 플랫폼은 Superpowers의 "프롬프트만으로 구성" 아키텍처에서 가능한 것이고, 우리처럼 Node.js CLI + 구조적 검증 게이트가 있으면 플랫폼별 포팅 비용이 큼
...
**현재 우리 구현:**
- Claude Code 기반 + Codex 플러그인으로 GPT 모델 통합 (리뷰 파이프라인)
- 구현/실행은 Claude 모델, 리뷰 Stage 2에서 GPT 엄격 감사 활용
- Anthropic API 장애 시 전체 파이프라인 중단 (폴백 체인 미적용)
